...
from tool.vote_mask import infer
cudnn.enabled = True
...
def train_phase(args):
    viz = Visdom(env="PDA_CA_2")  #env_name=PDA,progressive dropout attention
    model = getattr(importlib.import_module(args.network), 'Net')(args.init_gama, n_class=args.n_class) #network=network.resnet38_cls;init_gama=1,n_class=4
    print(vars(args))   #vars()返回当前作用域内的局部变量
...
if __name__ == '__main__':
    ...
    parser.add_argument("--env_name", default="PDA_CA", type=str)
    parser.add_argument("--model_name", default='PDA_CA', type=str)
    parser.add_argument("--n_class", default=2, type=int)
    parser.add_argument("--weights", default='init_weights/ilsvrc-cls_rna-a1_cls1000_ep-0001.params', type=str)
    parser.add_argument("--trainroot", default='F:/data/data_all/weak_suprvised_data/LUAD-HistoSeg/LUAD-HistoSeg/training', type=str)
    parser.add_argument("--testroot", default='F:/data/data_all/weak_suprvised_data/LUAD-HistoSeg/LUAD-HistoSeg/test/', type=str)

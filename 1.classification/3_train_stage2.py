...
class Trainer(object):
    def __init__(self, args):
        ...
        resume_stage1 = 'checkpoints/luad_add_ca_att_0515/stage1_checkpoint_trained_on_' + str(args.dataset) + '.pth'
    ...
    def load_the_best_checkpoint(self):
        checkpoint = torch.load('checkpoints/luad_add_ca_att_0515/stage2_checkpoint_trained_on_'+self.args.dataset + '.pth')
    ...
    def test(self, epoch, Is_GM):
        ...
        for i, sample in enumerate(tbar):
            ...
            pred = np.argmax(pred, axis=1)
            ## cls 4 is exclude
            pred[target==4]=4       #将grounndtruth的背景直接覆盖到预测上
            # print(pred.shape)
            # pred_png = Image.fromarray(np.uint8(pred[0]))
            # pred_png.save(os.path.join("F:/data/data_all/weak_suprvised_data/BCSS-WSSS/BCSS-WSSS/test/pred/",str(png_name) + '.png'))
            self.evaluator.add_batch(target, pred)
...
def main():
    ...
    parser.add_argument('--dataroot', type=str, default='F:/data/data_all/weak_suprvised_data/BCSS-WSSS/BCSS-WSSS/')
    parser.add_argument('--dataset', type=str, default='bcss')
    parser.add_argument('--savepath', type=str, default='checkpoints/luad_add_ca_att_0515/')

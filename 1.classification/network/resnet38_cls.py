...
from network.sync_batchnorm.attation import CA_Block
# from network.sync_batchnorm.amm import AMM
...
class Net(network.resnet38d.Net):
    def __init__(self, gama, n_class):
        ...
        self.ca_att = CA_Block(4096)

    def forward(self, x, enable_PDA):

        x = super().forward(x)  #[16,4096,28,28]
        # x = self.ca_att(x)
        gama = self.gama
        if enable_PDA:

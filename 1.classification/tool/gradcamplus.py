...
class GradCamPlusPlus:
    ...
    def __call__(self, input_img, target_category=None):
        ...
        '''grad_cam++'''
        grads_power_2 = grads_val ** 2
        grads_power_3 = grads_power_2 * grads_val
        # print(type(features))
        # print(features)
        sum_activations = np.sum(features[-1].cpu().data.numpy(), axis=(2, 3))
        eps = 0.000001
...
if __name__ == '__main__':
    ...
    args = get_args()
    args.image_path = 'F:/data/data_all/weak_suprvised_data/LUAD-HistoSeg/LUAD-HistoSeg/test/test_tmp/387709-10004-55944[1 0 0 1].png'
    import network.resnet38d
    model = 0
    weights_dict = 0
    weights_dict = torch.load(
        '../checkpoints/stage1_checkpoint_trained_on_luad'  + '.pth')

    model = getattr(importlib.import_module("network.resnet38_cls"), 'Net_CAM')( 4)
    model.load_state_dict(weights_dict)

    grad_cam = GradCamPlusPlus(model=model, feature_module=model.bn7, \
    ...
    for i in range(4):
        ...
        cam_show.save("F:/data/data_all/weak_suprvised_data/LUAD-HistoSeg/LUAD-HistoSeg/test/PM_multi_scale_single_maskbn7/cc"+str(i)+'.png')
        test = 1
